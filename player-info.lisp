(defun print-player-info (info)
  (format t "Accuracy: ~A ~%Eco: (T: ~A, CT: ~A)~%Half: (T: ~A, CT: ~A)~%Full: (T: ~A, CT: ~A)~%"
          (accuracy info)
...
(defparameter player-infos
  '(:Ainsleyy    (make-instance 'player-info
                  :accuracy 0.44
...
(defun get-player-info (symbol) (eval (getf player-infos symbol)))
